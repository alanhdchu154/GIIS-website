...
> 最後更新：2026-05-30 Foundation video Gate v2 + 3 upload candidates（Umi + Claude Code 一起 review lesson release gate，補強 `audit_lessons.py`：檢查 `style_manifest.json` subject theme、pause/answer slide 不可 hash-identical、visual category rhythm、`learning_check.json`、reviewer JSON 必須綁 current `script_sha`、fragile unicode slide labels、public-claim risk。`slide_kit.Deck` 現在自動寫 `style_manifest.json`，讓 Math/English/Biology 等課有可驗證的不同配色；`make_lesson.py` 修掉 `_silence` id 直接靜音的 bug，避免 answer reveal 有解說文字但 MP4 沒旁白。用新版 gate 跑 3 支 foundation videos：Algebra I M9 math/gold、English I M2 literature/sepia、Biology M1 science/teal，final combined audit：3 pass / avg score 100；release gate：ready 3 / needs_revision 0 / blocked 0。新增 English I M2、Biology M1 handoffs 與 lesson folders，建立 `teaching-videos/_audit/release-gate/approved_ready_to_upload.json`；`yt_queue.py upload --gate-ready --dry-run --max 4` 顯示 3 pending with human approval。未實際上傳 YouTube。）

> 前次：2026-05-30 Foundation lesson-video cc pipeline v0.1（補上 `tools/lesson-video/FOUNDATION_VIDEO_PIPELINE.md`、`cc_foundation_worker.py`、`foundation_video_gate.py` 與 npm scripts `lesson:cc-foundation` / `lesson:foundation-gate`。cc 慢的主因不是純啟動：one-line benchmark 約 2-3 秒，但 `--print` 沒有中間進度、repo cached context 約 29k tokens、MCP health 有 Netlify failed / Stripe needs auth、且 `.claude/settings.local.json` 有舊 `/Users/alanhdchu/GIIS/giis-website/.git` additionalDirectory。已修成實際 `/Users/alanhdchu/giis-website/.git`，並改用 stream-json wrapper + timeout + raw log，讓 cc 做影片時可見進度且不會無限沉默。目標流程固定為 Umi bounded handoff → cc production → foundation gate → Umi/Alan review → approved artifact → gated YouTube queue。）

> 前次：2026-05-30 Foundation lesson-video pilot ready（依 Alan 指示 AP 先不要管，因 Common App / AP-facing approval 尚未通過，不應再產生或上傳 AP-positioned videos。新增 `tools/lesson-video/FOUNDATION_VIDEO_PLAYBOOK.md`：改成 foundation-first、Umi teaching brief → Claude Code production mechanics → audit/release gate → Umi approval → upload。新增 `umi/handoffs/2026-05-30-foundation-video-v2-pilot.md`，第一個 bounded pilot 是 Algebra I Module 9 `Slope & Rate of Change`。Claude Code non-interactive run 有產出 initial folder 但未正常回報；Umi 接手審核，修正 answer-reveal slides，補 internal peer/student/source reviewer artifacts，render 出 `teaching-videos/algebra-i-module-9-slope-rate-change-v2/algebra_i_module_9_slope_rate_change_v2.mp4`（5.8 MB / 約 4.7 min）、`transcript.txt`、14 slides、contact sheet、5 checks。最終 `audit_lessons.py`：pass / score 100；`lesson_release_gate.py --check`：ready 1 / needs_revision 0 / blocked 0。尚未上傳；下一步必須走 human-visible approval + gated YouTube queue。另清掉 legacy `teaching-videos/algebra-i-module-9-slope/.cleaned` 與 stale `script.json.youtube`，避免已移除的 `bmyK05H8fFU` 被誤判為成功上傳。`PIPELINE_PAUSED.md` 保持排程/自動上傳暫停，只允許 Umi-approved non-AP foundation manual pilot。）

> 前次：2026-05-30 Broken YouTube and hard-blocking external resources removed（依 Alan 指示避免學生卡在死連結或付費/權限牆：新增 `tools/course-audit/remove_broken_youtube_resources.js`，依 audit 結果處理 902 條 broken course YouTube references；89 條用現有可用 `video2Url` 升為 primary `videoUrl`，813 條無可靠替代則清空 URL 並保留 removal note。`public/data/lessons-manifest.json` 移除失效 Algebra I Module 9 `bmyK05H8fFU`。另新增 `tools/course-audit/remove_blocking_external_resources.js`，清掉 102 條 hard-blocking external resource references：CommonLit、HBR、NoRedInk、Practice-It、AP Classroom、Grammarly、JSTOR、Google Docs/Slides/Drive、Canva、Criterion、Medium。Khan Academy 保留為 free nonprofit external resource；HubSpot Academy / Google Digital Garage 暫留為 soft-risk free account/certificate flow，待下一輪替換。重跑 `tools/course-audit/audit_external_resources.js` 後 course YouTube broken/unavailable = 0，GIIS lesson manifest YouTube failures = 0，hard-blocking hosts = 0；`npm run audit:pathways` 通過但 40 courses 有 missing-resource warnings，這是刻意用「不顯示死/卡住連結」換取「待補 vetted replacement」；`npm run build` 通過。新增 `server/scripts/sync-course-resources-from-json.js` 並用 `/tmp/giis-resource-sync` 在 Lightsail production 以 dry-run→apply 同步 Prisma module resource 欄位：first apply 718 changes，hard-blocking cleanup apply 90 changes，post-apply dry-run 0 changes / 0 missingCourses / 0 missingModules；live API scan：93 published courses / 373 YouTube refs / 285 unique IDs / 0 oEmbed failures / 0 hardBlockingRefs；live Netlify manifest：49 lessons，`bmyK05H8fFU` absent。未 reseed，未碰學生成績/progress/assignments。）
>
> 前次：2026-05-30 Lesson video strategy after graduation（依 Alan 問題重新判斷影片策略：日程上 2025-2026 Spring 已於 2026-05-22 結束、grades 2026-05-25 released、Class of 2026 diploma issue 2026-06-05；因此影片不再是已畢業學生補課急件，而是下一輪招生/新生 retention 的 trust asset。重跑 `npm run audit:lesson-video-inventory`：70 folders / 49 visible / 22 MP4 / full reviewer cascade 29/70 / AP missing reference packet 26 / visible_needs_posthoc_review 14。`npm run audit:lesson-manifest` 仍有 12 module-title mismatch warnings；`lesson_release_gate.py --pending --check`：18 evaluated / 3 ready / 15 needs_revision。新增 review note `umi/reviews/2026-05-30-lesson-video-strategy-after-graduation.md`：強烈建議不要恢復舊 cc-led batch pipeline；改成 Umi teaching brief → cc production mechanics → machine audit → Umi academic review → approved_ready_to_upload → upload。同步修 `tools/youtube-upload/README.md`，移除會誤導 operator 直接跑 `upload_lesson.py` 的舊 quickstart，改成 gated `yt_queue.py upload --gate-ready`。）
>
