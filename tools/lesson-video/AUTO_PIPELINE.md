...
- ZERO `.mp4`, `.mp3`, `.wav` (other than the music) in each folder; the active
  foundation pipeline renders only after Codex/Umi source review and gate checks
...
5. **Do NOT `git push`** from this legacy run. The active foundation pipeline
   owns upload/manifest updates only after Codex/Umi approval artifacts exist.
