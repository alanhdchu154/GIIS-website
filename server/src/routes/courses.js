...
// GET /api/courses/admin/all — list every course for Admin catalog management
router.get('/admin/all', authenticate, requireAdmin, async (req, res) => {
  const courses = await prisma.course.findMany({
    select: {
      ...
      isPublished: true,
      updatedAt: true,
      _count: { select: { modules: true, questions: true, quizQuestions: true, enrollments: true } },
    },
    orderBy: [{ department: 'asc' }, { name: 'asc' }],
  });
  res.json(courses);
...
// GET /api/courses/admin/:slug — Admin course detail, including unpublished courses
router.get('/admin/:slug', authenticate, requireAdmin, async (req, res) => {
  const course = await prisma.course.findUnique({
    where: { slug: req.params.slug },
    include: {
      modules: { orderBy: { order: 'asc' } },
      _count: { select: { questions: true, quizQuestions: true, enrollments: true } },
    },
  });
...
router.post('/', authenticate, requireAdmin, async (req, res) => {
  const { slug, name, nameZh, credits, department, type, gradeLevel, description, isPublished } = req.body || {};
...
router.patch('/:slug', authenticate, requireAdmin, async (req, res) => {
  const allowed = ['name', 'nameZh', 'credits', 'department', 'type', 'gradeLevel', 'description', 'isPublished'];
  const data = {};
...
// PATCH /api/courses/:slug/modules/:moduleId — update a module outline
router.patch('/:slug/modules/:moduleId', authenticate, requireAdmin, async (req, res) => {
  const course = await prisma.course.findUnique({ where: { slug: req.params.slug } });
  if (!course) return res.status(404).json({ error: 'Course not found' });
  ...
  try {
    ...
    if (e.code === 'P2002') return res.status(409).json({ error: 'Module order already exists for this course' });
    throw e;
  }
});
